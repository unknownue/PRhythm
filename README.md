...
- **Context Extraction**: Intelligently extracts relevant code context from PRs, improving analysis quality while reducing token usage
...
# Enable smart context extraction
python pipeline/analyze_pr.py --repo "owner/repo" --pr 123 --extract-context

# Extract context and save to separate file
python pipeline/analyze_pr.py --repo "owner/repo" --pr 123 --extract-context --save-context
```
...
The context extraction feature uses agent-based technology to intelligently extract only the most relevant code segments from PRs, improving analysis quality while reducing token usage:

```bash
# Extract context for a specific PR
python pipeline/extract_context.py --repo-path /path/to/local/repo --pr-number 123 --api-key your_api_key

# Extract context from a diff file
python pipeline/extract_context.py --repo-path /path/to/local/repo --diff-file path/to/diff.patch --api-key your_api_key

# Extract context without generating a summary
python pipeline/extract_context.py --repo-path /path/to/local/repo --pr-number 123 --api-key your_api_key --context-only

# Specify output directory
python pipeline/extract_context.py --repo-path /path/to/local/repo --pr-number 123 --api-key your_api_key --output-dir ./context_output
```

The context extraction process:
1. Analyzes the PR diff to identify changed code units
2. Extracts code relationships to understand dependencies
3. Generates code embeddings for semantic similarity search
4. Identifies key context needed to understand the changes
5. Creates a focused context summary for more efficient and accurate PR analysis